module.exports = function (client, cmd) {
    ...
    if (cmd.msg.mentions.users.map(user => user).length === 0) return cmd.msg.channel.send({ embeds: [{ "description": "❌ You need to Mention the User to Add/Remove their NSFW Access!", "color": "#f53b3b" }] }).then(msg => msg.delete({ timeout: 5000 }))
